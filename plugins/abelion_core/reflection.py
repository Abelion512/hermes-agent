...
def cache_messages(session_id=None, conversation_history=None, **kwargs):
    ...
    if session_id and conversation_history:
        ...
        # Parse and track links from conversation history
        try:
            from .link_tracker import track_link
            ...
            for msg in conversation_history:
                ...
                if isinstance(content, str) and content:
                    ...
                    for url in urls:
                        ...
                        track_link(url_clean)
        except Exception as e:
            logger.error(f"[abelion_autonomous] Link tracking in cache_messages failed: {e}")
...
def record_reflection(ctx, session_id=None, **kwargs):
    ...
    # Fallback to loading messages from state.db (crucial for gateway sessions)
    if not messages:
        try:
            from hermes_state import SessionDB
            db = SessionDB()
            messages = db.get_messages(session_id)
            logger.debug(f"[abelion_autonomous] Loaded {len(messages)} messages from state.db for reflection")
        except Exception as e:
            logger.warning(f"[abelion_autonomous] Failed to load messages from state.db: {e}")
    ...
    try:
        ...
        # Save to SQLite FTS5 Experience DB
        try:
            from .memory_store import save_experience
            save_experience(session_id, reflection_data)
        except Exception as se:
            logger.error(f"[abelion_autonomous] FTS Experience DB save failed: {se}")

        # Export to Obsidian Vault
        try:
            from .obsidian_exporter import export_session_to_obsidian
            export_session_to_obsidian(session_id, final_record)
        except Exception as oe:
            logger.error(f"[abelion_autonomous] Obsidian export failed: {oe}")
