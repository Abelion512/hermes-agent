...
class HermesACPAgent(acp.Agent):
    ...
    def _history_message_text(cls, message: dict[str, Any]) -> str:
        ...
        text = cls._flatten_history_text(message.get("content"))
        if not text:
            return ""

        # Filter out internal status messages that should not be visible in the chat buffer
        if (
            text.startswith("[MODE_UPDATE]")
            or text.startswith("[SESSION_STARTED]")
            or text.startswith("<session_context>")
        ):
            return ""

        # Strip internal <thinking> and <memory-context> tags that might have leaked
        # into the persisted content in previous versions or via other transports.
        import re

        text = re.sub(
            r"<(?:think|thinking|reasoning|thought|memory-context|REASONING_SCRATCHPAD)\b[^>]*>.*?</(?:think|thinking|reasoning|thought|memory-context|REASONING_SCRATCHPAD)>",
            "",
            text,
            flags=re.DOTALL | re.IGNORECASE,
        )
        # Also handle unterminated tags at the end of the text (common with reasoning)
        text = re.sub(
            r"<(?:think|thinking|reasoning|thought|memory-context|REASONING_SCRATCHPAD)\b[^>]*>.*$",
            "",
            text,
            flags=re.DOTALL | re.IGNORECASE,
        )
    ...
    def _history_tool_call_name_args(
        ...
        function = _fn if isinstance(_fn, dict) else {}
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        tool_progress_cb: Optional[Callable] = None
        reasoning_cb: Optional[Callable[[str], None]] = None
        step_cb: Optional[Callable] = None
        stream_delta_cb: Optional[Callable[[str], None]] = None
        ...
        if conn:
            ...
            message_cb = make_message_cb(conn, session_id, loop, agent=state.agent)
    ...
    def _cmd_model(self, args: str, state: SessionState) -> str:
        ...
        # Immediate health check for 9router/custom providers
        try:
            from plugins.abelion_core.health import verify_model_health

            health_error = verify_model_health(state.agent)
            if health_error:
                return (
                    f"Model switched to: {new_model}\n"
                    f"Provider: {provider_label}\n\n"
                    f"⚠️ CONNECTION WARNING: {health_error}"
                )
            else:
                return f"Model switched to: {new_model}\nProvider: {provider_label}\n✅ Connection: ONLINE"
        except Exception as e:
            logger.debug("Immediate health check failed: %s", e)
            return f"Model switched to: {new_model}\nProvider: {provider_label}"
