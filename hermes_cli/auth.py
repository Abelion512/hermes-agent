...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    ...
    "9router": ProviderConfig(
        ...
        name="9router",
        auth_type="api_key",
        inference_base_url="https://api.9router.ai/v1",
        api_key_env_vars=("NINEROUTER_API_KEY", "OPENROUTER_API_KEY"),
        ...
        while True:
            try:
                if fcntl:
                    ...
                    # Should be unreachable due to the outer if fcntl is None and msvcrt is None guard
                    pass
                break
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(
        os.getenv("HERMES_XAI_REFRESH_TIMEOUT_SECONDS", "20")
    )
